  ...
  return (
    <div className="space-y-8">
      <section className="rounded-[2rem] border border-stone-300/60 bg-[linear-gradient(135deg,_rgba(47,93,80,0.96)_0%,_rgba(95,130,103,0.92)_55%,_rgba(217,185,130,0.86)_100%)] p-8 text-white shadow-[0_24px_70px_-34px_rgba(29,51,42,0.75)] sm:p-10">
        ...
        <h1 className="mt-3 font-heading text-5xl">See every purchase turn into measurable savings.</h1>
        <div className="mt-8 grid gap-4 md:grid-cols-4">
          <StatCard icon={<ShoppingBag className="h-5 w-5" />} label="Purchases" value={dashboard.purchaseCount} />
          <StatCard icon={<Droplets className="h-5 w-5" />} label="Water saved" value={`${Math.round(dashboard.totalWaterSavedLiters)} L`} />
          <StatCard icon={<Recycle className="h-5 w-5" />} label="CO2 diverted" value={`${dashboard.totalCo2DivertedKg.toFixed(1)} kg`} />
          <StatCard icon={<Award className="h-5 w-5" />} label="Badges" value={dashboard.badges.length} />
        </div>
      </section>
      ...
      <section className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className="rounded-[2rem] border border-stone-300/60 bg-white/80 p-8 shadow-[0_18px_50px_-30px_rgba(55,45,32,0.45)] backdrop-blur">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-heading text-3xl text-stone-900">Purchase history</h2>
          </div>
          <div className="mt-6 space-y-4">
            {dashboard.purchases.length === 0 ? <p className="text-stone-600">No purchases yet. Use the quick purchase area below to test the full impact flow.</p> : null}
